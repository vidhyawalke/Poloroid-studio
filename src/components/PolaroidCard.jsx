...
export default function PolaroidCard({
  ...
  // Clamps pan coordinates based on image zoom
        ...
        >
          {image ? (
            <>
              ...
              {/* Hover overlay with clean "Upload New" button */}
            ...
            <div className="flex flex-col items-center text-center p-6 text-stone-400 hover:text-stone-500 transition-colors pointer-events-none select-none">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.2" stroke="currentColor" className="w-11 h-11 mb-2.5 text-stone-300">
              ...
              <p className="text-[9px] text-stone-400/80 mt-1">Scroll to Zoom • Drag to Crop</p>
            </div>
          )}
          ...
          <div className="flex justify-between items-end border-b border-stone-200/80 pb-1.5 w-full min-h-[2.4rem] relative">
            {isEditing ? (
              <input
                ...
                className={`bg-transparent border-0 outline-none text-stone-850 flex-1 min-w-0 ${getFontClass()} pb-0`}
              ...
              <div
                ...
                className={`text-stone-850 break-words flex-1 min-w-0 pr-2 cursor-text ${getFontClass()}`}
              >
            ...
            {/* Date Stamp */}
            {showDate && (
              <div className="font-date text-sm text-stone-500 tracking-wide select-none whitespace-nowrap pl-2">
                {date}
              </div>
            )}
          </div>
